...
function SpeechText({ locale, onTextChange }) {
  ...
  useEffect(() => {
    if (!worker.current) {
      ...
      worker.current = new Worker(
        new URL("../worker/audio-worker.js", import.meta.url),
        {
          type: "module",
        },
      );
          ...
          <div className="flex flex-col items-center px-4 gap-4 ">
            {status === null && (
              <>
                ...
                <button className="bg-lime-400">yo</button>
                <button
                  className="border px-4 py-2 rounded-xl bg-indigo-400 text-white hover:bg-blue-500 disabled:bg-blue-100 disabled:cursor-not-allowed select-none"
                  onClick={() => {
                    worker.current.postMessage({ type: "load" });
                    setStatus("loading");
                  }}
            ...
            {status === "ready" && (
              <div className="relative w-full flex justify-center ">
                ...
                <button
                  className="border rounded-xl px-2 absolute right-2"
                  onClick={() => {
                    recorderRef.current?.stop();
                    // recorderRef.current?.start();
                  }}
